<script setup lang="ts">
  ...
  function getRatingColor(): String{
    ...
    if (rating <= 50) return 'brand-blue'
    else if (rating > 50 && rating <= 70) return 'brand-orange'
    else return 'brand-magenta'
  }

  watch([isSaved, isLiked], ([newSaved,newLiked]) => {
...
<template>
  <div v-color="cardColors[level]" class="card relative w-250px h-350px flex flex-col justify-between 
    ...
    <div>
      ...
      <div class="card__body flex flex-col items-center gap-10px">
        ...
        <div class="card__text w-180px text-center">
          <h4 class="fg-foreground mb-5px">{{ title }}</h4>
    ...
    <div class="card__footer flex justify-between pr-15px mb-18px text-center font-500">
      <div>
        <span class="card__level block bg-brand-blue fg-white font-size-12px 
        px-15px py-4px">Level: {{ level }}</span>
        <span class="fg-gray font-size-12px block mt-2px">
          Avaliações: <span v-color="getRatingColor()" class="card__rating fg-brand-magenta">{{ rating }}%</span>
        </span>
      </div>
      <div class="flex text-size-30px gap-2px items-start">
        <UiIcon @click="isSaved = !isSaved" pointer name="bookmark" :variant="saveBtnVariant" color="brand-blue"></UiIcon>
        <UiIcon @click="isLiked = !isLiked" pointer name="heart" :variant="likeBtnVariant" color="brand-magenta"></UiIcon>
      </div>
    </div>
  </div>
</template>
