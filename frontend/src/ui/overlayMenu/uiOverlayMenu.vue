...
import { onClickOutside,
  useEventListener,
  useResizeObserver } from '@vueuse/core';
import type { PropType } from 'vue';
import { computed, provide, reactive, ref, useAttrs, watchEffect } from 'vue';
import { useOverlayStack } from '@/composables/useOverlayStack';
import type { OverlayMenuAlign,
  OverlayMenuOrigin,
  OverlayMenuSide } from './types';
...
const origin = computed(() => {
  ...
  if (hasSpace) {
    return [
      side,
      align,
    ] as [side: OverlayMenuSide, align: OverlayMenuAlign];
  }
  else {
...
onClickOutside(contentEl, () => props.closeOnClick && overlay.pop(), { ignore: [activatorEl] });
useResizeObserver([
  activatorEl,
  contentEl,
], updateRect);
...
<template>
  ...
  <Teleport :to="teleport">
    <div
      v-if="isVisible"
      ref="contentEl"
      class="ui-menu__content fixed z-999 inline-block"
      :style="[positionStyle, dimensionStyle]"
      @click="closeOnClickContent && close()"
    >
      <slot v-bind="bind" />
    </div>
  </Teleport>
